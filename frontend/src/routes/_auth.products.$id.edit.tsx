...
const variantSchema = z.object({
  id: z.number().optional(), // Added ID for existing variants
...
function EditProductPage() {
  ...
  useEffect(() => {
    if (product) {
      form.reset({
        ...
        basePriceVnd: product.basePriceVnd as any,
        ...
        variants: product.variants.map((v) => ({
          ...
          variantPriceVnd: v.variantPriceVnd as any,
          barcode: v.barcode || '',
  ...
  const updateMutation = useMutation({
    mutationFn: (data: ProductFormValues) => catalogApi.updateProduct(parseInt(id), data as any),
    onSuccess: () => {
  ...
  const generateMatrix = () => {
    ...
    for (const s of sizes) {
      for (const c of colors) {
        ...
        const exists = existingVariants.some(v => v.size === s && v.color === c);
        if (!exists) {
          newVariants.push({
            size: s,
            color: c,
    ...
    replace(newVariants);
    toast.success('Đã tạo các biến thể mới từ ma trận');
  };
  ...
  if (isLoadingProduct) {
    return <div className="flex h-full items-center justify-center">Đang tải thông tin sản phẩm...</div>;
  }
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Cập nhật thông tin sản phẩm và các biến thể</p>
        </div>
